...
export async function generateMetadata({ params }: TutorPageProps) {
  ...
  if (!tutor) {
    return {
      title: "Tutor não encontrado - Mensi",
    }
  }

  return {
    title: `${tutor.name} - ${tutor.title} | Mensi`,
    description: `Conheça ${tutor.name}, ${tutor.title}. Oferece tutoria em ${tutor.subjects.join(", ")}.`,
  }
}

// Gerar parâmetros estáticos para otimização
export function generateStaticParams() {
  return Object.keys(tutorsData).map((id) => ({
    id,
  }))
}
